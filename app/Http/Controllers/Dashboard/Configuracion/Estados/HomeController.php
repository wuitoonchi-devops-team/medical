...
{
    var $request,$model,$folder = "dashboard.configuracion.estados";
    ...
    public function index($country=true) {
        return view($this->folder.'.index',[
            'jsControllers'=>[
                0 => 'app/dashboard/configuracion/estados/HomeController.js'
            ],
            'country_id' => $country
        ]);
    }
    ...
    function enable_all() {
        $this->model->where('estatus',0)->get()->map(function($state){
            $state->estatus = 1;
            $state->save();
            return $state;
        });
    ...
    function disable_all() {
        $this->model->where('estatus',1)->get()->map(function($state){
            $state->estatus = 0;
            $state->save();
            return $state;
        });
